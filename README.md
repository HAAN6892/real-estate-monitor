...
```
├── .github/workflows/
│   ├── monitor.yml          # 매물 수집 워크플로우 (1일 2회)
│   └── policy-monitor.yml   # 정책 변경 감지 워크플로우 (주 1회)
├── index.html               # 대시보드 (GitHub Pages)
├── main.py                  # 매물 모니터링 스크립트
├── policy_monitor.py        # 정책 변경 감지 스크립트
├── settings.json            # 대시보드 기본 설정값
├── data.json                # 매매 데이터 (자동 생성)
├── data-rent.json           # 전월세 데이터 (자동 생성)
├── config.json              # API 키/설정 (Actions에서 자동 생성)
├── coord_cache.json         # 좌표 캐시 (자동 생성)
├── policy_cache.json        # 정책 캐시 (자동 생성)
...
서울 강남구·서초구·송파구·강동구·동작구·관악구, 경기 과천시·광명시·안양시·군포시·의왕시·성남시·하남시·구리시·용인 수지구·기흥구·광주시·수원 영통구·장안구
...
**완료**
- 지오코딩 법정동 제거 로직 적용 (동명이인 지역 바이어스 방지)
- 정책 변경 감지 배치 추가 (주 1회, 3개 정부 사이트 크롤링)
- 모바일 지도 팝업 UX 개선 (정보량 확대)
- 좌표 전체 리셋 및 재생성
- 키워드 제외 필터 추가 (오피스텔, 주상복합, 도시형, 빌라, 타운하우스)
- 카드/테이블에 세대수 표시
